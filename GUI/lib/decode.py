...
def main():
    ...
    parser.add_argument('--table_name', default="./outputs/tables/dnn")
    ...
    score_fun = {'Precision': precision_score,
                ...
                'F1 score': f1_score}
    diagnosis = ['1dAVb', 'RBBB', 'LBBB', 'SB', 'ST', 'AF']
    ...
    scores_list = []
    # generating full confusion matrix
    #
    nclasses = 6
    class_labels = ['1dAVb', 'RBBB', 'LBBB', 'SB', 'ST', 'AF']
    full_labels = ['1dAVb', 'RBBB', 'LBBB', 'SB', 'ST', 'AF', 'No Disease']
    # Initialize confusion matrix
    confusionMat = np.zeros((nclasses+1, nclasses+1), dtype=int)
    print(y_neuralnet)
    for i, y_pred in enumerate(y_neuralnet):
        trueLabel = y_true[i]
        if(np.sum(trueLabel < 1)):
            trueLabel = np.append(trueLabel, 1)
        else:
            trueLabel = np.append(trueLabel,[0])
        #print("before:",y_pred)
        if(np.sum(y_pred) < 1):
        ...
        #print("after:",y_pred)
        for k, vals in enumerate(trueLabel):
            if(vals == 1):
                for z, vals in enumerate(confusionMat[k]):
                    confusionMat[k][z] = confusionMat[k][z] + y_pred[z]
    # Convert the confusion matrix to a DataFrame
    df_confusion = pd.DataFrame(confusionMat, columns=full_labels)

    # Add a column for true class labels
    df_confusion.insert(0, 'True Disease', ['1dAVb', 'RBBB', 'LBBB', 'SB', 'ST', 'AF', 'No Disease'])

    # Save the DataFrame to an Excel file
    excel_filename = args.table_name + ".xlsx"
    df_confusion.to_excel(excel_filename, index=False)
    ...
    # Save results
    scores_all_df.to_excel(args.table_name + "_scores.xlsx", float_format='%.3f')
    scores_all_df.to_csv(args.table_name + "_scores.csv", float_format='%.3f')
    ...
    confusion_matrices.to_excel(args.table_name + "_confusion_matrices.xlsx", float_format='%.3f')
    confusion_matrices.to_csv(args.table_name + "_confusion matrices.csv", float_format='%.3f')
